...
class FileDataset:
    def __init__(self, file_path, selected_col_ids=None, dtypes=None, separator="\t", cached_index=False):
        ...
        assert os.path.exists(
            self.file_path), "Error: The local datafile {} not exists!".format(self.file_path)
        ...
        elif isinstance(selected_col_ids, str):
            self.selected_col_ids = [int(col_id) for col_id in selected_col_ids.split(",")]
        elif isinstance(selected_col_ids, list):
            self.selected_col_ids = selected_col_ids
        elif isinstance(selected_col_ids, omegaconf.listconfig.ListConfig):
            self.selected_col_ids = omegaconf.OmegaConf.to_container(selected_col_ids)
        else:
            self.selected_col_ids = selected_col_ids
        ...
        # # TODO Forace no ddp dataset, fix style
        # self.slice_id = 0
        # self.slice_count = 1
    ...
    def __getitem__(self, index):
        ...
        self.data_cnt += 1
        return [dtype(column_l[col_id]) for col_id, dtype in zip(self.selected_col_ids, self.dtypes)]
